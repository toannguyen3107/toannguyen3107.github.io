...
# Define the debugger address and port
debugger_address = 'http://localhost:36825'

# Get the list of tabs
response = requests.get(f'{debugger_address}/json')
tabs = response.json()

# Extract the WebSocket debugger URL and replace '127.0.0.1' with 'localhost'
...
# Command to attach to the new target
command = json.dumps({
    "id": 10,
...
# You have to send the screenshot command twice
ws.send(command)
response = json.loads(ws.recv())
